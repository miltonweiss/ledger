...
export default function ChatPage() {
    ...
    async function saveChat(messagesToSave) {
        ...
        const conversation = messagesToSave
            .filter(message => message.role && validRoles.includes(message.role))
            .map((message) => {
    ...
    useEffect(() => {
        async function loadChat() {
            const chat = await getSpecificChat(id);
            if (!chat) {
                router.push('/chat');
                return;
            }
            ...
            // Restore personality
            const personalityIndex = personalities.findIndex(p => p.name === chat.personality);
            if (personalityIndex >= 0) {
                setPersonality(personalityIndex);
            }

            // Restore messages (filter out any with missing/invalid roles)
            if (chat.conversation && chat.conversation.length > 0) {
                const validRoles = ['user', 'assistant', 'system'];
                const restored = chat.conversation
                    .filter(msg => msg.role && validRoles.includes(msg.role))
                    .map((msg, index) => ({
